class Player:
    ...
    def update(self, opponent_action, player_action):  # 这里没考虑draw
        ...
        if player_action[0] != "THROW":
            ...
            symbol = self.state[player_action[1]][0][1]
            ...
            if len(self.state[player_action[1]]) == 1:
                ...
                self.state[player_action[1]].remove(["player", symbol])

        if opponent_action[0] != "THROW":
            # opponent_action[1] => previous position
            symbol = self.state[opponent_action[1]][0][1]
            loc = {opponent_action[1]: ["opponent", symbol]}
            prev_state_record.update(loc)
            
            if len(self.state[opponent_action[1]]) == 1:
                del self.state[opponent_action[1]]
            else:
                self.state[opponent_action[1]].remove(["opponent", symbol])
        ...
        if player_action[0] == "THROW":
            if player_action[2] not in self.state:
                ...
                rps = player_action[1]
                old_rps = (self.state[player_action[2]])[0][1]
                if if_defeat(rps, old_rps) == "WIN":
                    for item in self.state[player_action[2]]:
                        if item[0] == "opponent":
        ...
        if opponent_action[0] == "THROW":
            if opponent_action[2] not in self.state:
                ...
                old_rps = (self.state[opponent_action[2]])[0][1]
                if if_defeat(rps, old_rps) == "WIN":
                    for item in self.state[opponent_action[2]]:
                        if item[0] == "player":
                            self.loss += 1  # opponent token eat player token
                        else:
                            self.win += 1  # opponent token eat opponent token
                    self.state[opponent_action[2]] = [["opponent", rps]]
                elif if_defeat(rps, old_rps) == "DRAW":
                    self.state[opponent_action[2]].append(["opponent", rps])
                else:
                    # if this token lose, then take this token away from the state
                    pass
        else:
